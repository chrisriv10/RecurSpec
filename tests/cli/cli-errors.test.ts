...
async function runCli(cwd: string, args: string[], retries = 1): Promise<CliRun> {
  try {
    ...
    // masquerades as a product failure and trips INTERNAL_ERROR assertions.
...
describe("recurspec CLI error paths", () => {
  ...
  it("reports missing executables without a stack trace", async () => {
    ...
    try {
      ...
      expect(res.code).toBe(1);
      expect(res.stdout, "code=" + String(res.code) + " stderr=" + res.stderr.slice(0, 2000)).toContain("INTERNAL_ERROR");
